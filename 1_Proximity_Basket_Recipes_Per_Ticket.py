...
df_o = df_order_baskets[['order_id', 'vectors']].rename(columns={'order_id': 'id'}).assign(source='Order')
df_ro = pd.concat([df_r, df_o])

tsne_plot(df=df_ro, title='Comparison between `Recipes` and `Orders`')

# The `recipes` are exceeding the average `order` basket, but still some of those are close to the products the customers are buying at Instacart.
# So, it's likely the recipes are not going to be fully covered with the products on any basket.
